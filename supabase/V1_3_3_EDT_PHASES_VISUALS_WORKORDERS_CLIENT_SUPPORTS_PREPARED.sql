...
-- Le lot est autonome: la production peut ne pas encore posseder edt_phases.
...
alter table public.edt_phases drop constraint if exists edt_phases_phase_type_v133_ck;
alter table public.edt_phases add constraint edt_phases_phase_type_v133_ck check (phase_type in ('installation','retrait')) not valid;
alter table public.edt_phases validate constraint edt_phases_phase_type_v133_ck;
alter table public.edt_phases enable row level security;
revoke all on table public.edt_phases from public,anon;
grant select on table public.edt_phases to authenticated;
drop policy if exists edt_phases_read_v133 on public.edt_phases;
create policy edt_phases_read_v133 on public.edt_phases for select to authenticated using (
  public.current_app_role() in ('Administrateur','Coordonnateur','Installateur')
  or exists (select 1 from public.suivi_des_edt e join public.utilisateurs u on u.auth_user_id=auth.uid() where e.id=edt_id and u.statut='Actif' and u.role in ('Client','Client-Admin') and e.client=u.client_id::text)
);
...
-- Backfill non destructif: Installation seulement. Aucun Retrait n'est invente.
insert into public.edt_phases(edt_id,nom,ordre,phase_type,statut,date_debut_prevue,date_debut_reelle,date_fin_reelle,closed_at,progression)
select e.id,coalesce(e.no_edt,'EDT-'||e.id::text)||' Installation',1,'installation',
  case
    when lower(coalesce(e.statut,'')) in ('terminé','termine','fermé','ferme','clos') then 'fermee'
    when lower(coalesce(e.statut,'')) in ('en cours','en_cours') then 'en_cours'
    else 'planifiee'
  end,
  coalesce(e.date_debut,case when e.date_debut_prevue::text ~ '^\d{4}-\d{2}-\d{2}' then left(e.date_debut_prevue::text,10)::date end),
  case when lower(coalesce(e.statut,'')) in ('en cours','en_cours','terminé','termine','fermé','ferme','clos') then e.date_debut::timestamptz end,
  case when lower(coalesce(e.statut,'')) in ('terminé','termine','fermé','ferme','clos') then coalesce(e.updated_at,now()) end,
  case when lower(coalesce(e.statut,'')) in ('terminé','termine','fermé','ferme','clos') then coalesce(e.updated_at,now()) end,
  case when lower(coalesce(e.statut,'')) in ('terminé','termine','fermé','ferme','clos') then 100 else 0 end
from public.suivi_des_edt e
where not exists(select 1 from public.edt_phases p where p.edt_id=e.id and p.phase_type='installation');
...
-- Machine d'etat canonique par phase. Le frontend ne possede aucun droit UPDATE.
...
revoke all on function public.transition_edt_phase_v133(bigint,text) from public,anon;
...
grant execute on function public.transition_edt_phase_v133(bigint,text) to authenticated;
