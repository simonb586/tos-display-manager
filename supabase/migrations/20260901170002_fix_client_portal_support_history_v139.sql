...
begin
 ...
 if p_section='photos' then
  ...
  select coalesce(jsonb_agg(to_jsonb(q)),'[]') into v_rows from(select a.id,a.occurred_at,a.action,a.module,a.entity_type,a.entity_id,a.campaign_id,a.edt_id,a.support_id,a.status from public.activity_events a where a.client_id=v_client and a.client_visible and(a.support_id=v_support or exists(select 1 from public.edt_supports es where es.edt_id=a.edt_id and es.support_id=v_support))order by a.occurred_at desc,a.id desc)q;
