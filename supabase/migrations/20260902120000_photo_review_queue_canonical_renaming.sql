...
create policy photo_review_audit_internal_read on public.photo_review_audit for select to authenticated using(
 public.current_app_role()='Administrateur' or (
  public.current_app_role()='Coordonnateur' and (
   client_id is null or (select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1) is null
   or client_id=(select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1)
  )
 )
);

drop policy if exists photo_review_internal_select on public.support_photos;
create policy photo_review_internal_select on public.support_photos for select to authenticated using(review_status is not null and (
 public.current_app_role()='Administrateur' or (
  public.current_app_role()='Coordonnateur' and (
   client_id is null or (select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1) is null
   or client_id=(select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1)
...
create policy photo_review_internal_insert on public.support_photos for insert to authenticated with check(source='mass_import' and review_status in('needs_review','unmatched') and support_id is null and client_id is null and not assignment_pending and target_storage_path is null and target_filename is null and target_support_id is null and target_client_id is null and public.current_app_role() in('Administrateur','Coordonnateur'));
drop policy if exists support_photos_field_write_v075 on public.support_photos;
create policy support_photos_field_write_v075 on public.support_photos for insert to authenticated with check(
 public.current_app_role() in('Administrateur','Coordonnateur','Installateur')
 and (client_id is null or (support_id is not null and exists(select 1 from public.infrastructures i where i.support_id=support_photos.support_id and i.client_id=support_photos.client_id)))
 and (review_status is null or (source='mass_import' and review_status in('needs_review','unmatched') and support_id is null and client_id is null and not assignment_pending and target_storage_path is null and target_filename is null and target_support_id is null and target_client_id is null and public.current_app_role() in('Administrateur','Coordonnateur')))
);
...
using(bucket_id='support-photos' and exists(select 1 from public.support_photos p where p.storage_path=storage.objects.name and p.assignment_pending and (public.current_app_role()='Administrateur' or (public.current_app_role()='Coordonnateur' and ((select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1) is null or p.target_client_id=(select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1))))))
with check(bucket_id='support-photos' and exists(select 1 from public.support_photos p where p.target_storage_path=storage.objects.name and p.assignment_pending and (public.current_app_role()='Administrateur' or (public.current_app_role()='Coordonnateur' and ((select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1) is null or p.target_client_id=(select u.client_id from public.utilisateurs u where u.auth_user_id=auth.uid() and u.statut='Actif' limit 1))))));
...
declare p public.support_photos%rowtype;i public.infrastructures%rowtype;seq integer;ext text;ymd text;name text;path text;actor uuid:=auth.uid();actor_role text;actor_client bigint;
begin
 ...
 if actor_role='Coordonnateur' and actor_client is not null and (i.client_id<>actor_client or (p.client_id is not null and p.client_id<>actor_client)) then raise exception 'SUPPORT_SCOPE_DENIED' using errcode='42501';end if;
...
create or replace function public.cancel_photo_review_assignment(p_photo_id bigint) returns void language plpgsql security definer set search_path='' as $$declare p public.support_photos%rowtype;actor uuid:=auth.uid();actor_role text;actor_client bigint;begin select u.role,u.client_id into actor_role,actor_client from public.utilisateurs u where u.auth_user_id=actor and u.statut='Actif' limit 1;if actor is null or actor_role not in('Administrateur','Coordonnateur')then raise exception 'PHOTO_REVIEW_FORBIDDEN' using errcode='42501';end if;select*into p from public.support_photos where id=p_photo_id for update;if not found then raise exception 'PHOTO_NOT_FOUND';end if;if actor_role='Coordonnateur' and actor_client is not null and coalesce(p.target_client_id,p.client_id) is distinct from actor_client then raise exception 'SUPPORT_SCOPE_DENIED' using errcode='42501';end if;update public.support_photos set assignment_pending=false,target_storage_path=null,target_filename=null,target_support_id=null,target_client_id=null where id=p.id;end$$;
create or replace function public.ignore_photo_review_item(p_photo_id bigint) returns jsonb language plpgsql security definer set search_path='' as $$declare p public.support_photos%rowtype;actor uuid:=auth.uid();actor_role text;actor_client bigint;begin select u.role,u.client_id into actor_role,actor_client from public.utilisateurs u where u.auth_user_id=actor and u.statut='Actif' limit 1;if actor is null or actor_role not in('Administrateur','Coordonnateur')then raise exception 'PHOTO_REVIEW_FORBIDDEN' using errcode='42501';end if;select*into p from public.support_photos where id=p_photo_id for update;if not found or p.review_status not in('needs_review','unmatched')then raise exception 'PHOTO_NOT_REVIEWABLE';end if;if actor_role='Coordonnateur' and actor_client is not null and p.client_id is distinct from actor_client then raise exception 'SUPPORT_SCOPE_DENIED' using errcode='42501';end if;update public.support_photos set review_status='ignored',statut_validation='Ignorée',validee_le=now()where id=p.id;insert into public.photo_review_audit(photo_id,actor_id,action,original_filename,ocr_text,ocr_confidence,proposed_support_id,previous_support_id,old_filename,old_storage_path,client_id)values(p.id,actor,'ignored',p.original_filename,p.ocr_text,p.ocr_confidence,p.proposed_support_id,p.support_id,p.nom_fichier,p.storage_path,p.client_id);return jsonb_build_object('ok',true,'photo_id',p.id);end$$;
