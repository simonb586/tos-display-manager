...
const file=`docs/review-edt-mission/terrain-${label}.json`;
...
try{
 ...
 f=(await managementQuery(`DO $$DECLARE s bigint;e bigint;e2 bigint;p bigint;p2 bigint;v bigint;BEGIN
 ...
 perform set_config('tdm.targeted.fixture',jsonb_build_object('supportId',s,'edtId',e,'edt2Id',e2,'phaseId',p,'phase2Id',p2,'visualId',v)::text,true);
 ...
 await productionBrowser(installer.session,async b=>{
  ...
  for(const mode of ['normal','without','inspection','enjeu','retrait']){
 ...
 const photos=await admin.client.from('support_photos').select('*').eq('support_id',support).order('id');assert.ifError(photos.error);assert.equal(photos.data.length,5);
 ...
 for(const [name,a] of [['Client',client],['Client-Admin',clientAdmin]]){
  const p=await a.client.rpc('photo_inventory_read',{p_table:'support_photos',p_filters:{support_id:support},p_limit:100});assert.ifError(p.error);assert.equal(p.data.rows.length,5);
 ...
 if(f){
  ...
  update support_photos set movement_history_id=null where support_id='${support}';delete from enjeux_terrain where support_id='${support}';delete from inspections_terrain where support_id='${support}';delete from historique_des_campagnes where support_id='${support}';delete from terrain_operations where support_id='${support}';delete from support_photos where support_id='${support}';delete from infrastructures where id=${f.supportId} and support_id='${support}';delete from campagne_visuels_formats where id=${f.visualId} and nom_visuel='${support}';delete from edt_phases where id in (${f.phaseId},${f.phase2Id});delete from suivi_des_edt where id in (${f.edtId},${f.edt2Id});END $$;`);
  evidence.fixtureCleaned=true;
 }
 for(const a of actors.reverse())await a.cleanup();save();
}
