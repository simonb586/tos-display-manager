...
window.mount=(kind='review',role='Administrateur')=>{fixture.calls=[];fixture.role=role;fixture.cancelled=false;fixture.photos=Array.from({length:48},(_,i)=>({id:i+1,source:'mass_import',review_status:'needs_review',original_filename:i===0?'unknown.jpg':(i%2?'SUP-A':'SUP-B')+'_'+i+'.jpg',storage_bucket:'support-photos',storage_path:'fixture/'+i+'.jpg',uploaded_at:'2026-06-01'}));root.render(kind==='review'?<PhotoReviewQueue key={++serial}/>:<DisplayMovementInventory key={++serial}/>);};
