...
import{availableKpi,displayKpiValue,unavailableKpi}from'../src/lib/module14Kpi.js';
...
for(const count of [0,1,27])assert.equal(displayKpiValue(availableKpi(count)),count.toLocaleString('fr-CA'));
for(const invalid of [null,undefined,Number.NaN,{},unavailableKpi(new Error('source'))])assert.equal(displayKpiValue(invalid),'Donnée non disponible');
for(const invalidText of ['NaN','null','undefined','[object Object]'])assert.notEqual(displayKpiValue({status:'available',value:{}}),invalidText);
...
const module14Service=fs.readFileSync('src/services/module14Service.js','utf8');
...
for(const marker of ['Tableau de bord','Marketing vs communication opérationnelle','RecentActivityWidget','Carte interactive','alerts','trends'])assert.ok(dashboard.includes(marker),marker);
...
for(const marker of ["'terrain'","'reports'","'reports_sent'","'reports_to_send'","'reports_errors'",'Diagnostic terrain','Rapports EDT','useDashboardSummary'])assert.ok(dashboard.includes(marker),marker);
