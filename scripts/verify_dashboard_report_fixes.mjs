...
await offlineBrowser('scripts/fixtures/dashboard-report-fixes-entry.jsx',async b=>{
 ...
 for(const [label,count] of [['Photos ajoutées','12'],['Infrastructures actives','1'],['EDT actifs','1'],['Enjeux ouverts','1'],['Campagnes marketing actives','1'],['Communications opérationnelles actives','1']]){assert.equal(await value(label),count,label);passed++}
