...
await offlineBrowser('scripts/fixtures/dashboard-p0-entry.jsx',async b=>{
 ...
 const timeoutMs=Number(fs.readFileSync('src/services/dashboardService.js','utf8').match(/DASHBOARD_TIMEOUT_MS\s*=\s*(\d+)/)[1]);
 const timeoutStart=Date.now();await b.evaluate("mount('timeout')");await b.waitFor('pending.length===6');await b.sleep(Math.max(0,timeoutMs-7000));await state('error');assert(Date.now()-timeoutStart<timeoutMs+3000);assert.equal(await b.evaluate('pending[5].signal.aborted'),true);assert(!(await values()).includes('Chargement…'));records.push(`Never-resolving request times out and aborts at the configured ${timeoutMs} ms bound`);
