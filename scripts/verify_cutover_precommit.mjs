...
const total=browser.reduce((n,r)=>n+r.count,0);assert.equal(total,628);
...
const result={date:new Date().toISOString(),browserCases:total,photoCases:30,historicalSuites:84,regressionSuites:9,secretScan:'PASS',diff:'PASS',files:candidates.files.length,browser,scope:'Local browser tests use simulated services; real production validation is required after deployment and before private cutover.'};
fs.writeFileSync(process.argv[2]||out+'/precommit-validation.json',JSON.stringify(result,null,2));
console.log(JSON.stringify({browserCases:total,photoCases:30,historicalSuites:84,regressionSuites:9,secretScan:'PASS',diff:'PASS'}));
