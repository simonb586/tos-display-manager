...
await offlineBrowser('scripts/fixtures/private-photo-entry.jsx',async({evaluate,waitFor,exceptions,consoleErrors})=>{
 ...
 await evaluate('photoFixture.deny=false;window.rpcPhoto={id:17,support_id:"HA-70059",storage_bucket:"support-photos",storage_path:"HA-70059/legacy.jpg",thumbnail_url:'+JSON.stringify(old)+'}');
 const listed=await run('listClientPhotos()');assert.equal(listed.error,undefined);assert.equal(await evaluate('window.lastPhotoRpc'),'client_portal_list_v1362');assert.ok(listed.value.rows[0].signed_thumbnail_url);assert.equal(await evaluate('photoFixture.calls.at(-1).bucket'),'terrain-photos');pass('Client photos use ownership projection and sign legacy Terrain reference');
 assert.deepEqual(exceptions,[]);assert.deepEqual(consoleErrors,[]);
},{realServices:['photoAccessService.js','photoWorkflowService.js','finalReportService.js','clientPortalService.js'],supabaseSource:`export const supabaseConfigured=true;export const supabase={rpc:async name=>{window.lastPhotoRpc=name;return {data:{rows:[window.rpcPhoto],total:1}}},storage:{from:bucket=>({createSignedUrl:(...args)=>window.signPhoto(bucket,...args),list:async()=>({data:[]}),upload:async()=>({error:null})})}};`});
