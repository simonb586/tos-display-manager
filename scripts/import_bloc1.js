...
const first = (...values) => values.map(txt).find(Boolean) || '';

const imports = [
  { table: 'infrastructures', file: 'infrastructures.json', key: 'support_id', map: r => ({
    support_id: first(r['Support ID'], r['support_id']),
  ...
  { table: 'liste_des_arrets', file: 'liste_des_arrets.json', key: 'no_arret', map: r => ({ no_arret: first(r["# d'Arrêt"], r['No arrêt'], r['no_arret']), emplacement_visibilite: txt(r['Emplacement/Visibilité']), type_support: txt(r['Type de support']), statut: txt(r['Statut']), visuel_affiche: txt(r['Visuel affiché']), photo_visuel_reel: txt(r['Photo du visuel réel']), raw_data: r })},
  { table: 'clients', file: 'clients.json', key: 'nom_client', map: r => ({ nom_client: first(r['Client'], r['nom_client']), type_client: txt(r['Type']), client_admin: txt(r['Client-admin']), statut: txt(r['Statut']), notes: txt(r['Notes']), raw_data: r })},
  { table: 'utilisateurs', file: 'utilisateurs.json', key: 'courriel', map: r => ({ nom: txt(r['Nom']), courriel: first(r['Courriel'], r['courriel']), role: txt(r['Rôle']), organisation: txt(r['Organisation']), statut: txt(r['Statut']), raw_data: r })},
  { table: 'suivi_des_edt', file: 'suivi_des_edt.json', key: 'no_edt', map: r => ({ no_edt: first(r['No EDT'], r['EDT'], r['no_edt']), campagne: txt(r['Campagne']), phase: txt(r['Phase']), client: txt(r['Client']), date_debut_prevue: txt(r['Date début prévue']), date_fin_prevue: txt(r['Date fin prévue']), dates_travaux: txt(r['Dates de travaux']), statut: txt(r['Statut']), supports_cibles: intVal(r['Supports ciblés']), supports_completes: intVal(r['Supports complétés']), photos_recues: intVal(r['Photos reçues']), avancement: pctVal(r['Avancement']), coordonnateur: txt(r['Coordonnateur']), commentaires: txt(r['Commentaires']), raw_data: r })},
  ...
  { table: 'centres_dinformation', file: 'centres_dinformation.json', key: 'nom_ci', map: r => ({ nom_ci: first(r['Nom du C.I.'], r['nom_ci']), nombre_cadres: intVal(r['Nombre de cadres']), photo: txt(r['Photo']), enjeux: txt(r['Enjeux']), date_derniere_intervention: txt(r['Date de la dernière intervention']), commentaires: txt(r['Commentaires']), raw_data: r })},
  ...
  { table: 'voitures_trains', file: 'voitures_trains.json', key: 'no_voiture', map: r => ({ no_voiture: first(r['#  Voiture'], r['# Voiture'], r['no_voiture']), statut_voiture: txt(r['Statut de la voiture']), support_id: txt(r['Support ID']), raw_data: r })},
  { table: 'photos', file: 'photos.json', key: 'photo_id', map: r => ({ photo_id: first(r['Photo ID'], r['photo_id'], `${txt(r['Support ID'])}_${txt(r['Date'])}_${txt(r['Action'])}`), support_id: txt(r['Support ID']), nom_fichier: txt(r['Nom fichier']), action: txt(r['Action']), date_photo: txt(r['Date']), utilisateur: txt(r['Utilisateur']), campagne: txt(r['Campagne']), visuel: txt(r['Visuel']), gps: txt(r['GPS']), statut: txt(r['Statut']), raw_data: r })},
  { table: 'bons_de_travail', file: 'bons_de_travail.json', key: 'no_bt', map: r => ({ no_bt: first(r['BT'], r['No BT'], r['no_bt']), type_bt: txt(r['Type']), support_id: txt(r['Support ID']), no_edt: txt(r['EDT']), priorite: txt(r['Priorité']), statut: txt(r['Statut']), assigne_a: txt(r['Assigné à']), date_cible: txt(r['Date cible']), client: txt(r['Client']), raw_data: r })},
...
function dedupeRows(table, rows, key) {
  if (!key) return rows;
  const map = new Map();
  let skipped = 0;
  rows.forEach((row, index) => {
    const rawKey = txt(row[key]);
    if (!rawKey) {
      skipped += 1;
      return;
    }
    if (map.has(rawKey)) skipped += 1;
    map.set(rawKey, row);
  });
  if (skipped > 0) console.log(`  ${table}: ${skipped} doublon(s) ou clé vide ignoré(s).`);
  return [...map.values()];
}

async function clearTable(table) {
  const { error } = await supabase.from(table).delete().gt('id', 0);
  if (error) throw new Error(`Erreur nettoyage ${table}: ${error.message}`);
}

async function insertChunks(table, rows, key, size = 500) {
  const uniqueRows = dedupeRows(table, rows, key);
...
async function main() {
  console.log('Début import Bloc 1 final...');
  const report = [];
  for (const item of imports) {
    const filePath = path.join(dataDir, item.file);
    if (!fs.existsSync(filePath)) {
      console.warn(`Fichier absent: ${item.file}`);
      report.push({ table: item.table, status: 'absent', count: 0 });
      continue;
    }
    const json = readJson(item.file);
    const rows = Array.isArray(json) ? json.map(item.map) : [];
    await clearTable(item.table);
    const imported = rows.length ? await insertChunks(item.table, rows, item.key) : 0;
    console.log(`✔ ${item.table}: ${imported} ligne(s) importée(s)`);
    report.push({ table: item.table, status: 'ok', count: imported });
  }
  console.log('\nRésumé import Bloc 1');
