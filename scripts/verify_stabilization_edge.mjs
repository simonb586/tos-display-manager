...
async function fixture(name,options={}){
 ...
 const callerProfile={role:options.role||'Administrateur',statut:'Actif',client_id:2};
 ...
 const admin={auth,from(table){let operation='select';const chain={select(){return chain},or(){return chain},eq(){return chain},upsert(){operation='update';return chain},update(){operation='update';return chain},maybeSingle(){if(table==='client_member_invitations')return Promise.resolve({data:{id:7,client_id:options.foreign?3:2,email:'target@example.test',requested_role:'Client',status:'pending'}});return Promise.resolve({data:callerProfile})},single(){return Promise.resolve({data:{id:1},error:options.profileError?{message:'Profile write failure'}:null})},then(resolve,reject){return Promise.resolve({data:null,error:operation==='update'&&options.profileError?{message:'Profile write failure'}:null}).then(resolve,reject)}};return chain}};
 const source=fs.readFileSync(`supabase/functions/${name}/index.ts`,'utf8').replace(/^import .*?;\s*/,'');
...
for(const name of ['invite-user','manage-user']){
 for(const [label,options,expected]of [['pending',{},200],['never invited',{noUser:true},200],['activated',{activated:true},409],['mail failure',{mailError:true},name==='invite-user'?400:500],['profile failure',{profileError:true},500],['unauthenticated',{unauthenticated:true},401],['unauthorized role',{role:'Client'},403]]){
