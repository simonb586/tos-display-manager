...
const file=`docs/targeted-improvements/${label}.json`;
...
try{
 ...
 for(const [name,a] of [['Client',client],['Client-Admin',clientAdmin]]){
  const p=await a.client.from('support_photos').select('*').eq('support_id',support);assert.ifError(p.error);assert.equal(p.data.length,5);
