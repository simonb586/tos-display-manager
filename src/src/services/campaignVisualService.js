...
export async function listCompatibleVisualsForSupport(support){ready();const format=String(support?.format_affichage||support?.format||support?.type_support||'').trim();if(!format)return[];const{data,error}=await supabase.from('campagne_visuels_formats').select('*, campagne:campagne_id(*)').eq('actif',true).ilike('format_support',format).order('phase').order('nom_visuel');if(error)throw error;return(data||[]).filter(v=>v.campagne?.publiee_terrain&&!['Annulée','Archivée'].includes(v.campagne?.statut));}
