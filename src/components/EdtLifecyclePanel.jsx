...
export default function EdtLifecyclePanel({edt,data,canManage,busy,run,loading=false}){
  ...
  return <section className="v07-card operations-wide edt-lifecycle">
    ...
    <div className="lifecycle-phases">{['installation','retrait'].map(type=>{
      ...
      return <article key={type} className="lifecycle-phase"><header><div><span>{type==='installation'?'01':'02'}</span><div><h3>{labels[type]}</h3><small>{phase?.statut||'Non créée'}</small></div></div><strong>{phase?.progression||0}%</strong></header>{phase?<>
