...
import{downloadCSV,downloadExcel,downloadPDF,professionalExportName,strictMatches}from'../lib/utils';
...
export default function ClientBusinessGrid({view,result,onLoad,onLoadAll,onOpenMap,readOnly=true}){
 ...
 const[query,setQuery]=useState(''),[filters,setFilters]=useState({}),[sortState,setSortState]=useState(null),[exporting,setExporting]=useState('');
 ...
 async function exportRows(kind){setExporting(kind);try{const permitted=await onLoadAll({search:query});const filtered=sortRows(permitted.filter(row=>matchesGridFilters(row,filters)),sortState),labels=Object.fromEntries(columns.map(key=>[key,LABELS[key]||key.replaceAll('_',' ')])),options={moduleName:view.label,labels,filters:{recherche:query},sortState,exportType:'Ensemble filtré autorisé'};if(kind==='csv')downloadCSV(professionalExportName(view.label,'csv'),filtered,columns.map(key=>({key,label:labels[key]})));if(kind==='xlsx')await downloadExcel(professionalExportName(view.label,'xlsx'),filtered,columns,options);if(kind==='pdf')await downloadPDF(professionalExportName(view.label,'pdf'),`${view.label} — résultats filtrés`,filtered,columns,options)}finally{setExporting('')}}
