...
export default function ActivityJournal({role}){
  ...
  return <div className="operations-page activity-journal">
    ...
    <div className="tableWrap"><table><thead><tr><th>Date</th><th>Utilisateur</th><th>Rôle</th><th>Action</th><th>Module</th><th>Objet</th><th>Campagne</th><th>EDT</th><th>Support</th><th>Client</th><th>Statut</th><th>Source</th></tr></thead><tbody>{rows.map(row=><tr key={row.id} onClick={()=>setSelected(row)}><td>{new Date(row.occurred_at).toLocaleString('fr-CA')}</td><td>{row.actor_email||'—'}</td><td>{row.actor_role||'—'}</td><td>{row.action}</td><td>{row.module}</td><td>{row.entity_type||'—'} {row.entity_id||''}</td><td>{row.campaign_id||'—'}</td><td>{row.edt_id||'—'}</td><td>{row.support_id||'—'}</td><td>{row.client_id||'—'}</td><td>{row.status||'—'}</td><td>{row.source_system}</td></tr>)}</tbody></table></div>
