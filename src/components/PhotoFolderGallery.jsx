...
export default function PhotoFolderGallery({photos,renderActions,onOpenSupport,onFolderChange,canDownload=true}) {
 ...
 return <section aria-label="Dossiers photos">
  <div className="editor-tabs"><button onClick={()=>{setFolder('');onFolderChange?.('');}}>Tous les dossiers</button>{selected&&<strong>{selected.label}</strong>}</div>
  ...
  {!selected&&!searching?<div className="photo-review-grid">{folders.map(f=><button key={f.key} onClick={()=>{setFolder(f.key);onFolderChange?.(f.key.startsWith('edt:')?f.label:'');}} aria-label={`Ouvrir ${f.label}`}><strong>📁 {f.label}</strong><span> — {f.photos.length} photos</span></button>)}</div>:<div className="photo-review-grid">{rows.map(photo=><article key={photo.id}>
