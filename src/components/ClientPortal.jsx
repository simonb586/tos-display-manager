...
function ClientPortalSession({profile={},onLogout,preview=null,onClose}){
 ...
 const requestId=useRef(0),infrastructureContext=useRef(null);
 ...
 async function openMap(supportId='',context={}){if(context.mapRows){infrastructureContext.current=context;setMapRows(context.mapRows);return;} const id=++requestId.current;setRefreshing(true);setError('');try{const rows=(await loadBusinessRows('Infrastructures',{targetUserId:previewMode?targetUserId:null})).rows;if(id===requestId.current)setMapRows(rows)}catch(e){if(id===requestId.current)setError(e.message)}finally{if(id===requestId.current)setRefreshing(false)}}
