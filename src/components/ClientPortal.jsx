...
function ClientPortalSession({profile={},onLogout,preview=null,onClose}){
 ...
 const load=useCallback(async(view,page=1,pageSize=25,filters={})=>{
  ...
  try{
   let next;
   if(previewMode){const base=previewSections[view.section]||{rows:[],total:0,page:1,page_size:25},wanted=navigationContext.supportId;const rows=wanted?(base.rows||[]).filter(row=>String(row.support_id||'')===String(wanted)):base.rows||[];next={...base,rows,total:rows.length};}
 ...
 async function openMap(){const id=++requestId.current;setRefreshing(true);setError('');try{const rows=previewMode?(previewSections.supports?.rows||[]):await listAllClientPortalSection('supports');if(id===requestId.current)setMapRows(rows)}catch(e){if(id===requestId.current)setError(e.message)}finally{if(id===requestId.current)setRefreshing(false)}}
