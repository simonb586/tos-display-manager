...
function DashboardContent({onNavigate,role,permission,summary,hidePending=false}){
 ...
 const cards=(DASHBOARD_CARDS[tab]||[]).filter(([key])=>kpis?available(key):!hidePending);
 ...
 {['general','marketing','operational'].includes(tab)&&summary.value&&!['Client','Client-Admin'].includes(role)&&<section className="executive-layout">{bars}<RecentActivityWidget onNavigate={onNavigate} role={role} permission={summary.value.permission||permission}/></section>}
 </div>;
}
function InternalDashboard(props){
 const summary=useDashboardSummary(props.scopeKey||props.role);
 return <DashboardContent {...props} summary={summary}/>;
}
export default function Module14Dashboard(props){
 if(props.clientProjection){
  const {identity,views=[],kpis,loading,error,refresh,refreshing}=props.clientProjection;
  const targets={'Carte interactive':'infrastructures','Photos et inventaire':'photos','Campagnes maîtres':'campaigns','Suivi des EDT':'edt','Enjeux des cadres et supports':'issues','Rapports EDT':'reports','Communications opérationnelles':'communications','Bons de travail':'requests','Clients':'members'};
  const navigate=target=>{const id=targets[target]||views.find(v=>v.label===target)?.id;if(id)props.onNavigate(id,target==='Carte interactive'?{map:true}:{});};
  return <DashboardContent hidePending role={identity?.role} onNavigate={navigate} summary={{value:kpis?{kpis,permission:props.clientProjection.permission}:null,loading,error,refresh,refreshing}}/>;
 }
