...
export default function OperationalCommandCenter({ dataStore, initialView = 'today', onNavigate }) {
  ...
  const alertTargets = ['Suivi des EDT','Campagnes maîtres','Photos et inventaire','Enjeux des cadres et supports','Rapports finaux'];
  ...
  return <section className="command-center">
    ...
    {view === 'alerts' && <div className="command-alerts">{[[model.overdue, 'EDT en retard', Clock3], [model.endingCampaigns, 'Campagnes se terminant', CalendarDays], [model.missingPhotos, 'Photos manquantes', MapPin], [model.urgentIssues, 'Enjeux urgents', AlertTriangle], [model.pendingReports, 'Rapports non envoyés', FileText]].map(([rows, label, Icon], index) => <button type="button" key={label} disabled={!onNavigate} onClick={()=>open(alertTargets[index])}><Icon/><span>{label}</span><strong>{rows.length.toLocaleString('fr-CA')}</strong></button>)}</div>}
