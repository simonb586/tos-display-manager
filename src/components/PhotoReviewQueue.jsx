...
export default function PhotoReviewQueue() {
 ...
 async function load(){const [photos,data]=await Promise.all([listPhotoReviewQueue(),loadPhotoImportCatalog()]);setCatalog(data);setRows(photos.map(p=>({...p,import_context:importContextForPhoto(p,data)})));}
 useEffect(()=>{load().catch(e=>setMessage(e.message));},[]);
 const counts=useMemo(()=>importRecognitionCounts(rows.map(p=>({recognition:p.import_context?.recognition,finalized:Boolean(p.import_finalized_at),error:p.review_status==='error'}))),[rows]);
 const visible=useMemo(()=>rows.filter(p=>{
  const r=p.import_context?.recognition,finalized=Boolean(p.import_finalized_at);
  const matches=filter==='all'||filter==='validated'&&finalized||filter==='ignored'&&p.review_status==='ignored'||!finalized&&(filter==='ready'?r?.ready:filter==='unmatched'?r?.unidentified:r?.states[filter]==='TO_REVIEW');
 ...
 async function execute(ids,finalize=false){
  if(active.current)return;active.current=true;setBusy(true);let saved=0;const errors=[];
  try{for(const id of ids){const photo=rows.find(p=>p.id===id);if(!photo||photo.import_finalized_at)continue;
    try{await savePhotoImportContext(id,photo.import_context);if(finalize){if(!photo.import_context.recognition.ready)throw Error('Informations encore à valider');await finalizeImportPhoto(id);}saved++;}catch(e){errors.push(`${photo.original_filename} : ${e.message}`);}
   }
 ...
 return <section className="photo-review-queue"><header><h2>Photos à valider</h2><p>Les originaux sont conservés. Seules les installations et retraits confirmés créent un mouvement.</p></header>
  ...
  <div className="review-grid">{shown.map(photo=><article key={photo.id}><label><input type="checkbox" checked={selected.has(photo.id)} disabled={busy||Boolean(photo.import_finalized_at)} onChange={e=>setSelected(current=>{const next=new Set(current);e.target.checked?next.add(photo.id):next.delete(photo.id);return next;})}/> Sélectionner</label>
   <ImportPhotoPreview photo={photo} onClick={()=>{setViewer(photo.id);setZoom(1);setRotation(0);}}/>
   <div className="review-card-body"><strong>{photo.original_filename||photo.nom_fichier}</strong><span>{photo.import_context?.recognition.values.support||'Support à identifier'}</span><span>{photo.import_finalized_at?'Validée':photo.import_context?.recognition.ready?'Prête à confirmer':photo.import_context?.recognition.unidentified?'À valider — non identifiée':'À valider'}</span><small>{photo.import_context?.recognition.values.date||'Date à confirmer'}</small>
    <button onClick={()=>{setViewer(photo.id);setZoom(1);setRotation(0);}}>Examiner / Corriger</button><button onClick={()=>original(photo)}>Fichier original</button>
   </div></article>)}</div>
  {!shown.length&&<p>Aucune photo pour ce filtre.</p>}
  ...
  {opened&&catalog&&<div className="review-viewer import-context-viewer" role="dialog" aria-modal="true" aria-label="Validation de la photo"><button onClick={()=>setViewer(null)}>Fermer</button>
   ...
   <div><PhotoImportContextEditor context={opened.import_context} catalog={catalog} onChange={context=>contextChanged(opened.id,context)} disabled={busy||Boolean(opened.import_finalized_at)}/>
    {!opened.import_finalized_at&&<div className="review-actions"><button disabled={busy} onClick={()=>execute([opened.id])}>Enregistrer les choix</button><button disabled={busy||!opened.import_context.recognition.ready} onClick={()=>execute([opened.id],true)}>Confirmer la photo</button><button disabled={busy} onClick={async()=>{try{await ignoreReviewPhoto(opened.id);await load();}catch(e){setMessage(e.message);}}}>Ignorer en conservant l’original</button></div>}
   </div>
  </div>}
 </section>;
}
